...
class BaseEnsemble(object):
    ...
    def plot_residuals(self, X_train, y_train, X_test, y_test):
        plt.scatter(
            self.predict(X_train).ravel(),
            self.predict(X_train).ravel() - y_train.ravel(), c='b', s=40,
            alpha=.5, label='train data prediction')
        plt.scatter(
            self.predict(X_test).ravel(),
            self.predict(X_test).ravel() - y_test.ravel(), c='g', s=40,
            alpha=.5, label='test data prediction')
    ...
    def scatterplot(self, X_test, X=None, y=None, title=None, filename=None):
        ...
        plt.fill_between(X_test.ravel(),
                         y_hat.ravel(), var, alpha=.3, color='b',
                         label='uncertainty')

        plt.fill_between(X_test.ravel(),
                         y_hat.ravel(), var2, alpha=.3, color='b')
    ...
    def mutli_dimenstional_scatterplot(self, X_test, y_test, X=None, y=None,
        ...
        for i, feature in enumerate(X_test.T):
            ...
            plt.fill_between(feature[s].ravel(), y_hat[s].ravel(), var,
                             alpha=.3, color='b', label='uncertainty')
            plt.fill_between(feature[s].ravel(), y_hat[s].ravel(), var2,
                             alpha=.3, color='b')
...
class RegressionEnsemble(BaseEnsemble):
        ...
        for i in range(self.num_models):
            try:
                new_regressor = self.model_type(
                    random_state=self.seed + i)  #random_state=self.seed+i)
            except:
    ...
    def predict(self, y_test, std=False):
        ...
        print(predictive_stds)
        print('prediction list {}'.format(prediction_list))
        return predictive_means, predictive_stds


class SubspaceEnsemble(RegressionEnsemble, BaseEnsemble):
    def __init__(self, num_models=None, model_type=None, seed=None,
                 num_drop_dimensions=None, num_features=None,
                 learning_rate=None):

        super().__init__(num_models=num_models, model_type=model_type,
                         seed=seed)
    ...
    def fit(self, X_train, y_train):
        ...
        for i in range(self.num_models):
            idx = np.random.choice(X_train.shape[0],
                                   X_train.shape[1] - self.num_drop_dimensions,
                                   replace=False)
...
class BootstrapEnsemble(RegressionEnsemble, BaseEnsemble):
    ...
    def __init__(self, num_models=None, model_type=None, seed=None,
                 keep_p=None, num_features=None, learning_rate=None):
    ...
    def fit(self, X_train, y_train, shuffle=False):
        ...
        for i in range(self.num_models):
            new_regressor = self.model_type()
            X_new, throwaway1, y_new, throwaway2 = train_test_split(
                X_train, y_train, test_size=self.keep_p,
                random_state=self.seed + i, shuffle=True)
            new_regressor.fit(X_new, y_new)
            self.regressor_list.append(new_regressor)
        return 'ensemble of {} {}s is hired and at the ready'.format(
            self.num_models, self.model_type.__name__)

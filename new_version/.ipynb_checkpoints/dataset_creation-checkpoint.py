...
class LinearDataset(base.BaseDataset):
	...
	def create_dataset(self):
		X, y = make_regression(n_samples=n_samples, n_features=1, noise=15,random_state=seed,shuffle=True)#,n_informative=1,bias=100)
		y = y/10 #+ 10
		return X,y

class NonlinearDataset(base.BaseDataset):
	def __init__(self,n_samples=n_samples,seed=seed,generating_function=None):
	...
	def base_generating_function(self,X):
		return np.sin(X).ravel()*10 + np.random.normal(0,1,size=X.shape[0])# + 10

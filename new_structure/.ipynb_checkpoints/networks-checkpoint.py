...
class EnsembleNetwork(base.BasePredictor,object):
	...
	def train_one_epoch(self, epoch_X, epoch_y, shuffle=False):
		if shuffle == True:
...
class GaussianLossNetwork_tmp(EnsembleNetwork):
	def __init__(
			self,
			num_neurons=[10, 10, 10],
			num_features=1,
...
class GaussianLossNetwork(EnsembleNetwork):
	...
	def error_graph(self):
		...
		#y_hat is a // output of prediction graph
		y_hat = tf.transpose(self.p_graph)
		std_hat = tf.maximum(tf.transpose(self.std_graph), 0.005)  #tf.square(self.std_graph)
		#s_hat = self.predict_graph[1]
...
class GaussianLearningRateNetwork(GaussianLossNetwork):
	...
	def error_graph(self):
		...
		#y_hat is a // output of prediction graph
		y_hat = tf.transpose(self.p_graph)
		#y_hat = tf.transpose(self.predict_graph)

		std_hat = tf.transpose((tf.maximum(self.std_graph, 0.001)))  #tf.square(self.std_graph)
		#s_hat = self.predict_graph[1]
